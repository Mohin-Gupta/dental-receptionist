...
function AcceptInviteContent() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    setError('');
    try {
      const response = await api.post('/auth/invites/accept', { token, name, password });
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex items-center justify-center px-4">
      <form onSubmit={submit} className="w-full max-w-md bg-gray-900 border border-gray-800 rounded-xl p-6">
        ...
        <h1 className="text-lg font-semibold text-white">Accept invite</h1>
        <p className="text-sm text-gray-400 mt-1 mb-5">Create your admin account.</p>
